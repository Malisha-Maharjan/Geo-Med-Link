...
def nepali_to_english_date(nepali_date_str):
    ...
    nepali_months = {
        ...
        'भदौ': 5, 'असोज': 6, 'कात्तिक': 7, 'मङ्सिर': 8,
...
# medical_news_soup = BeautifulSoup(
#     medical_news_page.text, 'html')
...
#     headline_link = "https://www.medicalnewstoday.com" + \
#         article.find('a', class_="css-1mttucs")['href']
...
#     data = {'english_date': date, 'headline': headline, 'image_url': image_url,
...
for article in articles:
    ...
    data = {"headline": headline, "image_url": image_url, "paragraph": paragraph,
            "headline_link": headline_link, "english_date": date}
    post = requests.post(url, json=data)
    print(data)
    if (post.json()['status'] == "error"):
        break
...
for article in articles:
    ...
    image_url = article.find("img")['data-src']
    data = {"headline": headline, "image_url": image_url, "paragraph": paragraph,
            "headline_link": headline_link, "nepali_date": date}
    post = requests.post(url, json=data)
    if (post.json()['status'] == "error"):
        break
print("ratopati")
